...
use super::{
    ...
    LocalityAssetError, MaterializationId, MaterializationRecord, MediaAssetError,
    MediaAssetMaterializer, OsmAccessAssetError, OsmPowerAssetError, PartitionResolutionError,
    PlannerError, ProjectEnrichmentAssetError, ProjectEnrichmentMaterializer, ReraAssetError,
    ...
    OSM_LOCALITY_BOUNDARY_FACTS_ASSET_ID, OSM_POWER_LINE_FACTS_ASSET_ID,
...
impl AssetDagExecutor {
    ...
    async fn promote_materialization_with_retry(
        ...
        loop {
            ...
            match self
                .materializations
                .compare_and_swap_current(record, expected_current)
                .await
            {
                Ok(true) => return Ok(()),
                Ok(false) => {
                    return Err(AssetDagExecutorError::CurrentPointerConflict {
...
impl BuiltInAssetExecutorRegistry {
    fn default_openestates() -> Self {
        ...
        executors.insert(
            static_asset_id(OSM_SOCIETY_ACCESS_FACTS_ASSET_ID),
        ...
        executors.insert(
            static_asset_id(SOCIETY_GOLD_SNAPSHOT_ASSET_ID),
            BuiltInAssetExecutor::SocietyGoldSnapshot,
        );
        Self { executors }
    }
    ...
    async fn execute(
        ...
        match self {
            ...
            Self::OsmSocietyAccessFacts => {
                ...
                let input = super::canonicalize_osm_society_access_input(
                    &context.dag.lake,
                    input,
                    canonical_record,
                    &context.options.source_inputs.source_entities,
                ...
                .await?;
                let input =
                    super::osm_society_access_facts_input(&input, &context.run_id.to_string())?;
                let materialization = execute_skill_fact_asset(context, &input).await?;
                Ok(ExecutedAsset::SkillFacts(materialization))
            }
            ...
            Self::SocietyFactSnapshot => {
                let parent_records = context
                    ...
                    .await?;
                let materialization =
                    SocietyFactSnapshotMaterializer::new(context.dag.lake.clone())
                        .materialize_for_run(
                            &parent_records,
                            &context.options.version,
                            context.run_id.clone(),
            ...
            Self::SocietyGoldSnapshot => {
                ...
                let parent_materializations = parent_records
                    ...
                    .collect();
                let materialization =
                    SocietyGoldSnapshotMaterializer::new(context.dag.lake.clone())
...
fn support_fact_records(
    ...
    parent_records
        .iter()
        .filter(|record| {
            record.asset_id.as_str() == SOCIETY_FACT_SNAPSHOT_ASSET_ID
...
impl ExecutedAsset {
    fn record(&self) -> &MaterializationRecord {
        match self {
            ...
            Self::SocietyGoldSnapshot(materialization) => &materialization.record,
        }
    }
}
...
pub enum AssetDagExecutorError {
    ...
    SocietyGoldSnapshot(SocietyGoldSnapshotMaterializeError),
...
impl fmt::Display for AssetDagExecutorError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SocietyGoldSnapshot(err) => write!(f, "KG society view execution failed: {err}"),
            ...
            Self::CurrentPointerConflict { asset_id } => {
                write!(f, "current pointer changed while promoting asset {asset_id}")
            }
        }
    }
}
...
impl AssetDagExecutorError {
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            | Self::SocietyFactSnapshot(SocietyFactSnapshotError::Lake(err))
            | Self::Rera(ReraAssetError::Lake(err))
            | Self::CanonicalNodes(super::CanonicalNodesError::Lake(err))
            | Self::SocietyGoldSnapshot(SocietyGoldSnapshotMaterializeError::Lake(err)) => {
                err.is_retryable()
            }
...
impl From<SocietyGoldSnapshotMaterializeError> for AssetDagExecutorError {
    fn from(err: SocietyGoldSnapshotMaterializeError) -> Self {
        Self::SocietyGoldSnapshot(err)
    }
}
...
impl From<OsmAccessAssetError> for AssetDagExecutorError {
    fn from(err: OsmAccessAssetError) -> Self {
        Self::OsmAccess(err)
    }
}
