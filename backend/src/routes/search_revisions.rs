...
impl SearchJourneyService {
    ...
    pub(crate) async fn start(
        ...
        let active = self.issue_active(
            ...
            output.collections.as_ref().to_vec(),
        )?;
    ...
    async fn revise_uncached(
        ...
        let refreshed_active = self.issue_active(
            ...
            refreshed_parent.collections.as_ref().to_vec(),
        )?;
        let catalog_delta = result_delta(
        ...
        if revision.outcome != SearchRevisionOutcome::Candidate {
            let (outcome, code, message) = match revision.outcome {
                SearchRevisionOutcome::RequireClarification => (
                    ...
                    if matches!(request.target, Some(SearchRevisionTarget::Predicate { .. })) {
        ...
        let active = self.issue_active(
            ...
            candidate.collections.as_ref().to_vec(),
        )?;
    ...
    async fn compile_revision(
        ...
        let explicit_target = request.target.clone();
        ...
        self.state
            ...
            .run_customer_compute(move || {
                let plan = crate::search::SearchEngine::new(&snapshot)
                    ...
                    .ok()?;
                // Already-unresolved signed intent may resume as an honest empty result.
                // A newly broken binding still preserves the parent for clarification.
                let valid = plan.branches.iter().all(|branch| {
                    !has_unresolved_required_branch(branch)
                        || intent_ast.branches.iter().any(|portable| {
                            portable.branch_id == branch.branch_id
                                && !portable.unresolved_requirements.is_empty()
                                && portable.unresolved_requirements
                                    == branch.unresolved_requirements()
                        })
                });
    ...
    fn issue_active(
        ...
        collections: Vec<crate::search::collections::JourneyCollection>,
    ...
    async fn selected_property_consequence(
        ...
        let mut explanation = if retained && !failed_predicate_ids.is_empty() {
            messages.selected_rejected_change.clone()
        } else if retained {
...
fn apply_explicit_target(
    ...
    match target {
            ...
            for patch in &mut revision.patches {
                match patch {
                    ...
                    TypedSearchRevisionPatch::AddAlternative {
                        ...
                        *inherit_branch_id = branch_id.clone();
                    }
        ...
        SearchRevisionTarget::Predicate {
            ...
            if let Some(binding) = branch
                .predicate_bindings
                .iter()
                .find(|binding| binding.predicate_id == *predicate_id)
            {
                let expression = expression.select_families(&[binding.family]);
                if !expression.has_terms() {
                    revision.outcome = SearchRevisionOutcome::RequireClarification;
                    revision.patches.clear();
                    return Ok(());
                }
...
fn has_unresolved_required_geography(plan: &CompiledSearchPlan) -> bool {
    plan.branches.iter().any(has_unresolved_required_branch)
}

fn has_unresolved_required_branch(branch: &crate::search::compiled_plan::GeoBranch) -> bool {
