...
pub fn compile_typed_revision(
    ...
    target: Option<&super::journey::SearchRevisionTarget>,
) -> TypedSearchRevision {
    ...
    let continuity =
        strip_configured_prefix(turn, &discourse.revision_continuity_prefixes).is_some();
    let standalone = !expand
        && fragment.semantic_fingerprint != parent.semantic_fingerprint
        && !correction
        && !exclusion
        && !continuity
        && !targets_every_branch(turn)
        && targeted_branch_index(turn, parent.branches.len()).is_none()
        && fragment.branches.iter().all(|branch| {
            discourse
                .revision_standalone_family_sets
                .iter()
                .any(|required| {
                    !required.is_empty()
                        && required.iter().all(|family| {
                            branch.predicate_bindings.iter().any(|binding| {
    ...
    // A fresh request must not inherit branch targeting or old preferences.
    if target.is_none()
        && (explicit_replace || standalone)
        && fragment
            .branches
            .iter()
            .any(|branch| branch.predicates.has_terms())
    {
        ...
        return TypedSearchRevision {
            ...
            patches: alternatives
                ...
                .map(|(index, branch)| TypedSearchRevisionPatch::AddAlternative {
                    ...
                    inherit_branch_id: parent.branches[0].branch_id.clone(),
    ...
    for patch in &revision.patches {
        match patch {
            ...
            TypedSearchRevisionPatch::AddAlternative {
                ...
                let inherited_branch = parent
                    .branches
                    .iter()
                    .find(|branch| branch.branch_id == *inherit_branch_id)?;
                let geo_families = [
                ...
                {
                    let mut inherited = inherited_branch.predicates.clone();
                ...
                let inherited_ranking = &inherited_branch.ranking_intent;
