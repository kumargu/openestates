...
impl SearchRuntimeSnapshot {
    pub fn new(
        ...
        let society_names: HashMap<String, String> = societies
            ...
            .collect();
        let browse_cards = properties
            .iter()
            .filter(|property| property.is_listable())
            .map(|property| {
                let fallback_society_entity_id = society_node_id(&property.society_id);
                let society_entity_id = search_index
                    .society_entity_id_for_property(&property.id)
                    .map(str::to_string)
                    .unwrap_or(fallback_society_entity_id);
                let society_name = society_names
                    .get(&property.society_id)
                    .cloned()
                    .unwrap_or_else(|| property.title.clone());
                let card = BrowsePropertyCard::from_runtime(
                    property,
                    society_entity_id,
                    society_name,
                    &bundle.fact_index,
                    &discovery_config.card_signals,
                );
                (property.id.clone(), card)
            })
            .collect();
        let discovery_shelves = crate::discovery::hydrate_shelf_rankings(&browse_cards);
...
pub struct CachedSearchOutput {
    ...
    pub collections: Arc<[crate::search::collections::JourneyCollection]>,
