...
async fn dag_plan_captures_freshness_and_dependency_propagation() {
    ...
    let missing_support = plan_entry(&plan, "google_places_weekly");
...
async fn dag_plan_fans_all_current_support_partitions_into_global_kg_lineage() {
    ...
    let home_state_facts = materialization_in_partition(
        "home_state_signals",
        AssetStage::Silver,
        "2026-07-13",
        vec![rera_facts.materialization_id.clone()],
        now,
        AssetPartition::global(),
    );
    ...
    write_current(&materializations, &groundwater_facts).await;
    let metro_station_facts = materialization_in_partition(
...
async fn dag_plan_errors_when_partition_policy_requires_missing_run_key() {
    ...
    let lake = LakeStore::local(root.path()).unwrap();
    let materializations = AssetMaterializationStore::new(lake);
    let planner = AssetPlanner::new(
        AssetRegistry::new(vec![asset(
            "partitioned_source",
            AssetStage::Raw,
            &[],
            RefreshCadence::Daily,
        )
        .with_partition_policy(backend::assets::AssetPartitionPolicy::from_run_keys(
            &["dt", "source"],
        ))])
        .unwrap(),
        materializations,
    ...
    assert!(matches!(
        ...
        PlannerError::Partition(PartitionResolutionError::MissingRunPartitionKey {
            ...
            ref key,
            ..
        }) if missing_asset_id == &asset_id("partitioned_source") && key == "source"
    ));
}
