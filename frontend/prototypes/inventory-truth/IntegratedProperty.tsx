...
type Load = { key: string; catalog: InventoryCatalog; detail: InventoryDetail };
...
export function IntegratedProperty() {
  ...
  useEffect(() => {
    ...
    void getCatalog(controller.signal).then(async catalog => {
      ...
      if (!controller.signal.aborted) setLoaded({ key, catalog, detail });
    }).catch((cause: unknown) => {
  ...
  return <div className="inventory-integrated">
    <div className="inventory-preview-bar">
      <span>Design preview · example unit & prices</span>
      {loaded && <label>Scenario<select aria-label="Scenario" value={scenario} onChange={event => {
        const next = new URLSearchParams(params);
        next.set("scenario", event.target.value);
        setParams(next, { preventScrollReset: true });
      }}>{loaded.catalog.homes.map(item => <option key={item.home.id} value={item.home.id}>{item.home.scenario}</option>)}</select></label>}
    </div>
    {ready && summary ? <PropertyPage homeIdentity={{
      ...
      detail: {
        summary: <div className="inventory-integrated__ask">
          <span>{summary.ask ? "Asking price" : "No active asking price"}</span>
          {summary.ask && <strong>{askingPrice(summary.ask)}</strong>}
        </div>,
        label: action,
        title: copy.title,
        content: <InventoryReceipts key={`${scenario}:${ready.catalog.snapshot_id}`} detail={ready.detail} />,
      },
