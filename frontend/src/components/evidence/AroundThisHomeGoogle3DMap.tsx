...
export function AroundThisHomeGoogle3DMap(props: AroundThisHomeMapProps) {
  ...
  function toggleExpanded() {
    if (expanded) setFlat(false);
    onToggleExpanded();
  }

  return (
    <div
      className={`nearby-map nearby-map--google-3d${expanded ? " is-expanded" : ""}`}
      role="region"
      aria-label="Nearby evidence map"
      aria-busy={!ready}
      data-map-renderer="google-3d"
    >
      <div ref={containerRef} className="nearby-map__canvas nearby-map__canvas--google-3d" />
      <div className="nearby-map__actions">
        {cameraMode === "evidence" && (
          <button type="button" onClick={backToHome}>Back to home</button>
        )}
        {expanded && (
          <button type="button" onClick={() => setFlat((current) => !current)}>
            {flat ? "3D" : "2D"}
          </button>
        )}
