...
export function buildBaselinePlanInputs(propertyPriceInr: number): PlanInputs {
  ...
  return {
    propertyPriceLakh,
    downPaymentLakh: Math.max(10, Math.round(propertyPriceLakh * 0.27 / 5) * 5),
    loanRate: 8.4,
    loanTenureYears: 20,
...
export function calculateUpfrontCash(inputs: PlanInputs): number {
  ...
  return inputs.downPaymentLakh * LAKH + purchasePrice * PURCHASE_COST_RATE;
}
